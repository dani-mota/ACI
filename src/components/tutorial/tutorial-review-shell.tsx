...
import { ReviewClient } from "@/components/role-builder/review-client";
import type { RoleBuilderPipelineResult } from "@/lib/role-builder/pipeline";

export function TutorialReviewShell() {
  ...
  if (error) {
    return (
      <div className="min-h-screen bg-background flex flex-col items-center justify-center gap-4">
        ...
        <button
          onClick={() => router.push("/tutorial/roles/new")}
  ...
  return (
    <ReviewClient
      result={result}
      saveEndpoint="/api/tutorial/roles"
      redirectPath={(slug) => `/tutorial/roles/${slug}`}
    />
  );
}
