...
function generateExecutiveSummary(
  ...
  // Guard: if no subtest results, return a minimal summary
  if (!subtestResults || subtestResults.length === 0) {
    return {
      overviewNarrative: `${firstName}'s assessment for the ${roleName} role is pending scoring. Detailed results will appear once assessment data is processed.`,
      keyInsight: "Assessment data is not yet available for this candidate.",
      strengths: [],
      devAreas: [],
      layers: (["COGNITIVE_CORE", "TECHNICAL_APTITUDE", "BEHAVIORAL_INTEGRITY"] as LayerType[]).map((layer) => ({
        layer, name: LAYER_INFO[layer].name, color: LAYER_INFO[layer].color, avg: 0,
      })),
      percentile,
      passed,
      distance,
      rampWeeks: predictions?.estimatedRampWeeks ?? "—",
      supervisionLevel: predictions?.supervisionLevel ?? "—",
