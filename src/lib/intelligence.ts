...
export function generateLeadership(results: SubtestScore[]): IntelligencePanel {
  ...
  if (mc >= 75 && ej >= 75) {
    narrative = `Exceptional self-awareness (${mc} scaled score) combined with strong ethical judgment (${ej} scaled score) creates a foundation for trusted leadership. This individual knows what they don't know and makes principled decisions under pressure. They're likely to earn peer respect quickly and can be trusted to escalate problems rather than hide them. ${fr >= 70 ? `With fluid reasoning at the ${fr} scaled score, they can also navigate complex, ambiguous situations that lack clear precedent — a hallmark of senior leaders.` : "Their leadership strength is character-driven rather than analytically-driven; pair them with strong analytical thinkers for best results."}`;
    ...
    narrative = `Both metacognitive calibration (${mc} scaled score) and ethical judgment (${ej} scaled score) are below average. This combination elevates risk — the individual may not recognize their limitations AND may not make the right call when it matters. This is the profile most associated with quality escapes that could have been prevented by speaking up. Close supervision and structured decision-making protocols are non-negotiable if advancing this candidate.`;
    ...
    narrative = `Moderate leadership indicators with metacognitive calibration at the ${mc} scaled score and ethical judgment at the ${ej} scaled score. This profile doesn't raise red flags but doesn't signal exceptional leadership potential either. Most manufacturing roles don't require exceptional leadership from day one — with mentoring, structured feedback, and incremental responsibility increases, leadership capacity typically develops over 18-24 months for candidates in this range.`;
    keyPoints.push("Adequate for current role responsibilities — no immediate concerns");
    if (fr >= 75) keyPoints.push(`Strong reasoning ability (${fr} scaled score) supports growth into technical leadership`);
...
export function generatePractical(results: SubtestScore[], ceilings?: CeilingInfo[]): IntelligencePanel {
  ...
  if (strengths.length >= 3) {
    narrative = `Broad technical strength across ${strengths.join(", ")}. This candidate has the cognitive toolkit for complex technical roles — they can visualize problems in 3D (SV: ${sv}), work with numbers and tolerances (QR: ${qr}), understand how systems interact (SD: ${sd}), and spot anomalies before they become defects (PR: ${pr}). This rare combination predicts rapid mastery of advanced manufacturing processes and suggests they'll be the person others come to for troubleshooting within 6 months.`;
    ...
    narrative = `Average technical aptitude profile (SD: ${sd}, MR: ${mr}, SV: ${sv}, QR: ${qr}, PR: ${pr}). This profile is typical and adequate for standard manufacturing roles. No construct stands out as a strength or concern. The candidate should be able to handle routine technical demands but may need support for complex troubleshooting. Target training at the lowest-scoring construct for maximum improvement — small gains at the bottom have more operational impact than marginal gains at the top.`;
...
export function generateTeamDynamics(results: SubtestScore[]): IntelligencePanel {
  ...
  if (teamScore >= 70) {
    narrative = `Strong team contributor profile (composite team score: ${teamScore} scaled score). High ethical judgment (${ej}) means they'll speak up about problems even when it's uncomfortable — this is the single strongest predictor of healthy team culture. ${cf >= 70 ? `Cognitive flexibility (${cf}) allows them to adapt to different teammates' approaches without friction.` : ""} ${mc >= 70 ? `Good self-awareness (${mc}) means they'll ask for help when needed rather than causing delays or quality issues silently.` : ""} This person is likely to earn trust quickly, contribute positively to team culture, and become the kind of teammate that senior operators specifically request on their shift.`;
    ...
    narrative = `Average team dynamics profile (composite: ${teamScore} scaled score). No significant concerns but no standout team strengths either. They'll integrate into most teams without friction, contributing reliably to established workflows. ${ej < 50 ? `Monitor for tendency to go along with the group rather than raising concerns — ethical judgment at ${ej} scaled score suggests they may defer to authority even when they notice problems.` : ""} ${cf < 50 ? `May need time to adapt to new team configurations or process changes (CF: ${cf} scaled score).` : ""} ${mc < 50 ? `Moderate self-awareness (${mc}) — may occasionally over- or under-estimate their contribution to team objectives.` : ""}`;
...
export function generateSelfAwareness(results: SubtestScore[]): IntelligencePanel {
  ...
  if (mc >= 75) {
    narrative = `Exceptional metacognitive calibration (${mc} scaled score) — this candidate accurately assesses what they know and what they don't. This is one of the most undervalued capabilities in manufacturing: the ability to say "I'm not sure" instead of guessing. ${lv >= 70 ? `Combined with high learning velocity (${lv}), they identify gaps quickly and fill them efficiently. This is the ideal trainee profile — they learn fast AND they know when they need to learn.` : `Even with moderate learning speed (${lv}), their self-awareness means they seek help at the right moments, preventing costly mistakes during ramp-up. They'll learn at a normal pace but with far fewer errors along the way.`} ${fr >= 70 ? `High fluid reasoning (${fr}) adds another dimension — they not only know what they don't know, they can figure it out independently when resources are available.` : ""}`;
...
export function generateOnboarding(results: SubtestScore[], roleName?: string): IntelligencePanel {
  ...
  if (lv >= 75) {
    ...
    narrative = `Below-average learning velocity (${lv} scaled score). Estimated productive ramp: ${rampWeeks} weeks${roleName ? ` for ${roleName}` : ""}. Estimated full independence: ${independenceWeeks} weeks. Extended onboarding with additional mentoring recommended. ${prl >= 60 ? "Positive: once they learn a procedure, they follow it consistently — the investment in extended training pays off through long-term reliability." : "Extra attention needed — slow learning combined with inconsistent procedure-following requires patient, structured training with frequent assessment checkpoints."} Consider pairing with an experienced mentor for the first ${independenceWeeks} weeks. The cost of extended onboarding should be weighed against the candidate's other strengths.`;
    keyPoints.push(`Estimated productive contribution: ${rampWeeks} weeks | Full independence: ${independenceWeeks} weeks`);
