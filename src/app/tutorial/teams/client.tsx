...
import { getDemoDataset } from "@/lib/team-composition/demo-data";
import { getMissions } from "@/lib/team-composition/mission-data";
...
export function TutorialTeamsClient({ industry }: { industry: TutorialIndustry | null }) {
  const dataset = useMemo(() => getDemoDataset(industry), [industry]);
  const missions = useMemo(() => getMissions(industry), [industry]);
  const allProfiles = useMemo(() => Object.values(dataset.profiles), [dataset]);
  const [selectedTeamId, setSelectedTeamId] = useState(dataset.teams[0].id);
  const [activeTab, setActiveTab] = useState<ViewTab>("dashboard");
  const [teamDropdownOpen, setTeamDropdownOpen] = useState(false);

  const selectedTeam = useMemo(
    () => dataset.teams.find((t) => t.id === selectedTeamId)!,
    [dataset, selectedTeamId]
  );
  const members = useMemo(() => dataset.getMembers(selectedTeam), [dataset, selectedTeam]);
  const candidates = useMemo(() => dataset.getCandidates(selectedTeam), [dataset, selectedTeam]);
    ...
    <div className="px-6 py-6">
      ...
      <div className="mb-6">
        ...
        <div className="flex items-center justify-between">
          ...
          {/* Team selector — hidden when mission tab active */}
          {activeTab !== "mission" && (
                ...
                <>
                  ...
                  <div className="absolute right-0 top-full mt-1 w-72 z-50 rounded-lg border border-slate-700/50 bg-[#0f1729] shadow-xl shadow-black/40 overflow-hidden">
                    {dataset.teams.map((team) => {
                      const teamMembers = dataset.getMembers(team);
                      return (
                        <button
                          key={team.id}
                          onClick={() => {
